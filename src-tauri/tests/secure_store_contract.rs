...
fn stores_reads_updates_and_deletes_secrets_in_keyring() {
    ...
    assert_eq!(saved.service, "job-hunt");
    assert_eq!(saved.key, "email.account.smtpPass");
    assert_eq!(saved.uri, "keyring://job-hunt/email.account.smtpPass");
    assert_eq!(
...
fn replaces_known_setting_secrets_with_keyring_references() {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "service": "job-hunt",
            "uri": "keyring://job-hunt/email.account.smtpPass"
        })
    );
    assert_eq!(
        value["imapPass"],
        json!({
            ...
            "service": "job-hunt",
            "uri": "keyring://job-hunt/email.account.imapPass"
        })
    );
    assert_eq!(
