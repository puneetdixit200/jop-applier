...
fn sends_email_account_settings_to_email_check_sidecar() {
    ...
    let request_path = std::env::temp_dir().join(format!(
        "job-hunt-email-check-request-{}.json",
        std::process::id()
    ));
...
fn resolves_keyring_email_secret_references_for_sidecar_workflows() {
    ...
    job_hunt_lib::secure_store::save_secret(
        ...
        UpsertSetting {
            ...
            value: SettingValue::Object(json!({
                ...
                "oauthClientSecret": {
                    ...
                    "service": "job-hunt",
                    "uri": "keyring://job-hunt/email.account.oauthClientSecret"
                },
                "oauthRefreshToken": {
                    "secretRef": "email.account.oauthRefreshToken",
                    "service": "job-hunt",
                    "uri": "keyring://job-hunt/email.account.oauthRefreshToken"
                },
                "signature": "Asha"
            })),
        },
    ...
    .expect("save email account setting");
    let request_path = std::env::temp_dir().join(format!(
        "job-hunt-email-secret-ref-request-{}.json",
        std::process::id()
    ));
    let command = capture_request_sidecar(&request_path);
...
fn sends_email_match_context_to_email_check_sidecar() {
    ...
    let request_path = std::env::temp_dir().join(format!(
        "job-hunt-email-match-request-{}.json",
        std::process::id()
    ));
    let command = capture_request_sidecar(&request_path);
...
fn sends_email_account_settings_to_cold_email_sidecar() {
    ...
    .expect("save email account setting");
    let request_path = std::env::temp_dir().join(format!(
        "job-hunt-cold-email-request-{}.json",
        std::process::id()
    ));
    let command = capture_request_sidecar(&request_path);
...
fn sends_email_account_and_follow_up_context_to_follow_up_sidecar_and_persists_result() {
    ...
    let request_path = std::env::temp_dir().join(format!(
        "job-hunt-follow-up-request-{}.json",
        std::process::id()
    ));
