...
function runtimeNotificationAdaptersFromEnv(
  ...
  const telegramBotToken = env.TELEGRAM_BOT_TOKEN ?? env.JOB_HUNT_TELEGRAM_BOT_TOKEN;
  const telegramChatId = env.TELEGRAM_CHAT_ID ?? env.JOB_HUNT_TELEGRAM_CHAT_ID;
  if (
    telegramBotToken &&
    telegramChatId &&
  ...
  const discordWebhookUrl = env.DISCORD_WEBHOOK_URL ?? env.JOB_HUNT_DISCORD_WEBHOOK_URL;
  if (
    discordWebhookUrl &&
...
function emailNotificationConfigFromEnv(env: NodeJS.ProcessEnv) {
  const to = splitEnvList(
    env.NOTIFICATION_EMAIL_TO ?? env.JOB_HUNT_NOTIFICATION_EMAIL_TO,
  );
  const smtpHost = env.SMTP_HOST ?? env.JOB_HUNT_SMTP_HOST;
  const smtpUser = env.SMTP_USER ?? env.JOB_HUNT_SMTP_USER;
  const smtpPass = env.SMTP_PASS ?? env.JOB_HUNT_SMTP_PASS;
  const fromEmail =
    env.EMAIL_FROM ?? env.FROM_EMAIL ?? env.JOB_HUNT_EMAIL_FROM;

  if (to.length === 0 || !smtpHost || !smtpUser || !smtpPass || !fromEmail) {
    return null;
  }

  const smtpPort = numberEnv(env.SMTP_PORT ?? env.JOB_HUNT_SMTP_PORT, 587);
  const smtpSecure = booleanEnv(env.SMTP_SECURE ?? env.JOB_HUNT_SMTP_SECURE);
  const account: EmailAccountConfig = {
    provider: emailProviderEnv(env.EMAIL_PROVIDER ?? env.JOB_HUNT_EMAIL_PROVIDER),
    smtpHost,
    smtpPort,
    smtpSecure,
    smtpUser,
    smtpPass,
    imapHost: env.IMAP_HOST ?? env.JOB_HUNT_IMAP_HOST ?? "",
    imapPort: numberEnv(env.IMAP_PORT ?? env.JOB_HUNT_IMAP_PORT, 993),
    imapSecure: booleanEnv(env.IMAP_SECURE ?? env.JOB_HUNT_IMAP_SECURE ?? "true"),
    imapUser: env.IMAP_USER ?? env.JOB_HUNT_IMAP_USER ?? smtpUser,
    imapPass: env.IMAP_PASS ?? env.JOB_HUNT_IMAP_PASS ?? smtpPass,
    fromName:
      env.EMAIL_FROM_NAME ?? env.FROM_NAME ?? env.JOB_HUNT_EMAIL_FROM_NAME ?? "job-hunt",
    fromEmail,
  };
...
function browserStealthConfigFromEnv(env: NodeJS.ProcessEnv) {
  const proxyList = (env.BROWSER_PROXY_LIST ?? env.JOB_HUNT_BROWSER_PROXY_LIST ?? "")
    .split(",")
    .map((server) => server.trim())
    .filter(Boolean)
    .map((server) => ({ server }));
  const sessionRoot = env.BROWSER_SESSION_ROOT ?? env.JOB_HUNT_BROWSER_SESSION_ROOT;
  const persistCookies =
    env.BROWSER_PERSIST_COOKIES ?? env.JOB_HUNT_BROWSER_PERSIST_COOKIES;

  return createDefaultStealthConfig({
    rotateProxy: booleanEnv(env.BROWSER_ROTATE_PROXY ?? env.JOB_HUNT_BROWSER_ROTATE_PROXY),
    proxyList,
...
function browserSessionStoreFromEnv(env: NodeJS.ProcessEnv, sessionRoot: string) {
  const key = env.BROWSER_SESSION_KEY ?? env.JOB_HUNT_BROWSER_SESSION_KEY;
  if (!key) {
    return undefined;
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const runtime = createSidecarRuntime({
    logging: {
      logDir: process.env.JOB_HUNT_LOG_DIR ?? defaultLocalEventLogDir(),
    },
  });
